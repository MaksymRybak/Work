...
var debug = require('debug')('HipConnectTester:hipchat');
...
module.exports = function (addon) {
  ...
  return {
    ...
    sendMessage: function (clientInfo, roomId, msg, opts, card) {
      addon.logger.info("Sending message to HipChat ", clientInfo.clientKey);
    ...
    getRoomMessages: function (clientInfo, roomId) {
      return request(clientInfo, {
        method: 'GET',
        resource: '/room/' + roomId + '/history'
      });
    },

    pushGlanceDataToRoom: function (clientInfo, roomId, moduleKey, glance) {
      return request(clientInfo, {
        method: 'POST',
    ...
    pushGlanceDataToGroup: function (clientInfo, roomId, moduleKey, glance) {
      return request(clientInfo, {
        method: 'POST',
        resource: '/addon/ui',
        body: {
    ...
    createGlance: function (clientInfo, roomId, glance) {
      var glanceKey = glance['key'];
      if (!glanceKey || glanceKey.length === 0) {
        throw new Error('key is required');
      }
      return request(clientInfo, {
        method: 'PUT',
        resource: '/room/' + roomId + '/extension/glance/' + glanceKey,
        body: glance
      });
    },

    deleteGlance: function (clientInfo, roomId, glanceKey) {
      if (!glanceKey || glanceKey.length === 0) {
        throw new Error('key is required');
      }
      return request(clientInfo, {
